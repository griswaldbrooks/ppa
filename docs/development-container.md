...
The full set of volume maps can be read in the `compose.dev.yml`.

#### git
Commits can be done from directly within the container, with the same host user.
```shell
username@project-dev:~/ws/src$ git config --list
...
remote.origin.url=git@github.com:organization/project.git
...
```shell
mkdir -p ~/.project/ccache
sudo chown -R $(id -u):$(id -g) ~/.project/ccache
```
