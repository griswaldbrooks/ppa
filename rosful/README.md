# rosful
Demo environment for consuming a rosless package.
...
username@rosful-dev:~/ws$ ros2 launch platform_description display.launch.py
```
...
```
username@rosful-dev:~/ws/src$ git config --list
...
remote.origin.url=git@github.com:griswaldbrooks/ppa.git
...
```
username@rosful-dev:~/ws/src$ rviz2
```
...
```
mkdir -p ~/.rosful/ccache
sudo chown -R $(id -u):$(id -g) ~/.rosful/ccache
```
